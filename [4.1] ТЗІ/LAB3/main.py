...
tq1 = [[0x2, 0x8, 0xB, 0xD, 0xF, 0x7, 0x6, 0xE, 0x3, 0x1, 0x9, 0x4, 0x0, 0xA, 0xC, 0x5],
...
def write_to_file(filename, data):
    try:
        with open(filename, "w") as file:
            file.write(data)
        print(f"Дані записані у файл {filename}")
    except Exception as e:
        print(f"Помилка при записі у файл {filename}: {str(e)}")

input_filename = "text.txt"
plaintext = read_file(input_filename)

key = input("Input the key 128 bit (Hexadecimal): ")
...
decrypt_filename = "encrypted.txt"
encrypted_text = read_file(decrypt_filename)

if encrypted_text:
    decrypted_text = decrypt(encrypted_text, key)
    decrypted_filename = "decrypted.txt"
    write_to_file(decrypted_filename, decrypted_text)
